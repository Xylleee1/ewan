...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_notification'])) {
    ...
    if ($recipient_type === 'all') {
        $res = mysqli_query($conn, "SELECT email, full_name FROM users WHERE email IS NOT NULL AND email != ''");
    } elseif ($recipient_type === 'students') {
        $res = mysqli_query($conn, "SELECT email, full_name FROM users WHERE role='student' AND email IS NOT NULL AND email != ''");
    } elseif ($recipient_type === 'faculty') {
        $res = mysqli_query($conn, "SELECT email, full_name FROM users WHERE role='faculty' AND email IS NOT NULL AND email != ''");
    } elseif ($recipient_type === 'overdue') {
    ...
    foreach ($recipients as $recipient) {
        ...
        $stmt = mysqli_prepare($conn, "INSERT INTO notifications (recipient_email, recipient_name, subject, message, sent_by) VALUES (?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "ssssi", $email, $name, $subject, $body, $_SESSION['user_id']);
        mysqli_stmt_execute($stmt);
    }
    
    $message = "Notification queued for " . count($recipients) . " recipients.";
    $msgClass = "success";
